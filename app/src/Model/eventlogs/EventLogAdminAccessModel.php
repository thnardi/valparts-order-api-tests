...
class EventLogAdminAccessModel extends Model
{
  public function add(EventLogAdminAccess $eventLog)
  {
    ...
    $parameters = [
        ':id_event_log_type'    => $eventLog->id_event_log_types_admin_access,
  ...
  public function getAll($order, $filtro, int $offset = 0, int $limit = PHP_INT_MAX)
  {
    $sql =
      ...
      FROM
        event_logs_admin_access
        LEFT JOIN admin_ancora ON admin_ancora.id = event_logs_admin_access.id_admin_ancora ";
    if ($filtro == 1) {

    }
    if ($filtro == 2) {
      $sql .= "WHERE event_logs_admin_access.id_event_log_type = 1 ";
    }
    if ($filtro == 3) {
      $sql .= "WHERE event_logs_admin_access.id_event_log_type = 2 ";
    }
    if ($order == 1) {
